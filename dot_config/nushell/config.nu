...
# version = "0.100.0"
source secrets.nu
source ~/.config/nushell/themes/custom_theme.nu
...
def --env z [path?:string] {
    if ($path == null) {
        ...
        cd (^zoxide query $path | str trim)
    }
}

def --env zi [] {
    cd (^zoxide query -i | str trim)
}

# Initialize zoxide (add this to your config)
if (which zoxide | is-empty) {
    print "zoxide not found. Installing..."
    cargo install zoxide
}


alias n_tips = glow ~/.config/nvim/tips.md
alias lz = lazygit
...
source scripts/external-commands.nu
source scripts/custom_search.nu
source scripts/custom_ls.nu
